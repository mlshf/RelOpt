...
class Greedy(Algorithm):
    ...
    def Run(self):
        ...
        self.system.Update(use_metamodel=False)

        while not self._checkStopCondition():
            self.Step()

            self.numOper = self.numOper + 1
            print "Iteration =",self.numOper, ';', self.system

        print "\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\n"
        print "Best solution: cost =", self.system.cost, self.system

        print "--------------------------------------\n"
        Algorithm.time = time.time() - Algorithm.time
        self.stat.AddExecution(Execution(copy.deepcopy(self.system), self.numOper, Algorithm.time, Algorithm.timecounts, Algorithm.simcounts))
        self.Clear()
    ...
    def _checkStopCondition(self):
        if StopCondition.maxIter != -1:
            if (self.system != None and self.system.CheckConstraints()) or self.numOper > StopCondition.maxIter:
                return True

        if StopCondition.maxIterWCH != -1:
            if self.system != None and self.numOper >= StopCondition.maxIterWCH:
                return True
